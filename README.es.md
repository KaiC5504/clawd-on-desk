...
- **ZCode** — hooks opcionales de estado y `PermissionRequest` bloqueante mediante `~/.zcode/cli/config.json` → `hooks.events.*` (instálalos desde Ajustes → Agentes o ejecuta `npm run install:zcode-hooks`); Clawd muestra globos manuales de Permitir/Denegar, mientras la automatización global y por sesión permanece aplazada. Clawd conserva los ajustes explícitos globales o por hook `enabled:false` y no se registra sobre un hook `PermissionRequest` de terceros
