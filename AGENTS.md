...
- DeepSeek Harness 首发是 **web profile / Windows-first experimental / plugin-only**；rc.6 已于 2026-08-29 完成 macOS / Windows x64 源码运行的真实 API-backed session/approval smoke（#962）；rc.2 Windows packaged-app 证据仅覆盖安装、web boot 与直接 bridge client 的 state/Allow/Deny 往返（#938），不得合并宣称 rc.2 安装包的真实 API 会话已验证。Clawd-managed in-process plugin 只监听公开 `session/created`、`session/event`、`session/disposed` 与 blocking `approval/request`。状态按 session FIFO 上报并携带上游 seq；Clawd 不读取 `$DSH_HOME/storages` projection。普通审批进入独立 DSH `/permission` adapter，只支持人工 Allow Once / Deny；204 / 断连 / DND / disabled 都由 plugin `next()` 交还 DSH 原生 web answerer。`ask_user_question` 始终留在 DSH 原生 provider；DSH 不进入 `KNOWN_PERMISSION_AGENTS`，auto-tools / unattended / per-session grant 全部 DEFER。安装器通过官方 `dsh plugin --profile web add/remove` 管理 immutable marker-owned generation，foreign 同名包 fail closed；当前支持一张 verified 版本契约表：优先 `@deepseek-ai/dsh@0.1.1-rc.2`（integrity `sha512-UP1UIh6q3Gme/yXRn/QL2P8IsVlv8Shpg22TRJIZPsCRWLm4CBiA1MUvXmJAfsOEETBMLAl+xWPtFw6ICsN3wg==`），并保留 `@deepseek-ai/dsh@0.1.0-rc.6`（integrity `sha512-brpZfED7ieRa2PQ5tUxMhHrM1pb2CmKFVM/f6yMULBDMicahk+Z2OsHgTwTDnoiZm23Ftu9rQz0NN4pflaoJcg==`）；Install/Repair 按检测到的 host 版本（无 CLI 时按 owned marker）选择契约，Uninstall / 手动 npx 命令按 marker 契约选择，未列入表的版本禁止 mutation，且预发布版本不做宽泛 range 匹配。`47f9438` 仅是源码调查基线，不代表任何 tag/commit 映射。
...
- Codex CLI official hooks 已接入；JSONL 轮询仍是 fallback，用于 hook 不可用、hook 未覆盖事件（如 WebSearch / compaction / abort）和历史兼容。本机 `SessionStart` 冷启动还受 `autoStartWithCodex` 控制：新安装默认关闭，v17 升级保持原先的开启行为，损坏/非权威 prefs 在当前进程内 fail closed，WSL / WSL interop / Remote SSH 永不走冷启动。Windows `commandWindows` 由 PowerShell 解析：legacy 直接命令必须用 `& "node" ...` call operator；本机 Windows stable 入口使用 `& "node" "codex-hook.js"` 直连 call-operator 形态（2026-09-04 起：旧的内联 PowerShell dispatcher “解码并执行”命令行被 Windows Defender ML 判为 Trojan:Win32/Commando.A!ml，见 clawd-on-desk#986）；环境变量由 codex-hook.js 启动时自读 UTF-8/Base64 `codex-hook.js.windows.run` 数据 sidecar 注入，JSON manifest 只供 Doctor。不得改回裸 `"node" "hook.js"`（会 exit 1）、二次 `powershell.exe -File`、无 BOM `.ps1` 或内联 dispatcher
